...
BitcoinExchange::BitcoinExchange() : _dbPath(""), _inputPath("") {}

BitcoinExchange::BitcoinExchange(std::filesystem::path inPath,
...
BitcoinExchange::BitcoinExchange(const BitcoinExchange &src)
    : _dbPath(src._dbPath), _inputPath(src._inputPath),
      _datetimeMap(src._datetimeMap) {}
...
BitcoinExchange::parse_database(std::filesystem::path dbPath,
                                std::string delim) {
...
std::time_t BitcoinExchange::parseDateTime(const std::string &dateString,
...
std::string BitcoinExchange::formatDatetime(const std::time_t &timePoint,
                                            const std::string &format) {
...
void BitcoinExchange::print_database(std::map<time_t, double> db) {
  for (auto it : db)
    std::cout << "Date: " << formatDatetime(it.first, FORMAT)
              << ", price: " << it.second << std::endl;
}

std::time_t BitcoinExchange::split_keyvalue(double &value,
  ...
  if (key == -1) {
    std::cerr << "Error: invalid date => " << line.substr(0, delim_pos)
              << std::endl;
    return -1;
  }
  try {
...
#ifdef DEBUG
        std::cout << "Bound date: " << formatDatetime(res->first, FORMAT)
                  << ", bound price: " << res->second << std::endl;
#endif
        std::cout << formatDatetime(key, FORMAT) << " => " << value << " = "
                  << (value * price) << std::endl;
      } else {
